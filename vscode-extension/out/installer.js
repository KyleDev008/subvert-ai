...
class Installer {
    ...
    async checkPrerequisites() {
        ...
        if (!pythonCommand) {
            throw new Error('Python 3.8+ not found. Please install Python from https://python.org and ensure it is in your PATH.');
        }
    ...
    async setupEnvironment() {
        ...
        await new Promise((resolve, reject) => {
            const venvProcess = child_process.spawn(pythonPath, ['-m', 'venv', '.venv'], { cwd: serverPath });
...
`;
        ...
        await new Promise((resolve, reject) => {
            const pipProcess = child_process.spawn(pipCmd, ['install', '-r', 'requirements.txt'], { cwd: serverPath });
